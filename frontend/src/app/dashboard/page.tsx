...
export default function DashboardPage() {
  ...
  // Log every render
  console.log(
    `[dash RENDER] ready=${ready} rehydrated=${rehydrated} agreementId=${agreementId} isPartyB=${isPartyB} wallet=${walletAddress?.slice(0, 10)}`,
  );

  useEffect(() => {
    console.log("[dash INIT effect] running, didInit=", didInit.current);
    if (didInit.current) return;
    didInit.current = true;
    console.log("[dash INIT] dispatching rehydrateSession");
    dispatch(rehydrateSession());
    // Log localStorage
    if (typeof window !== "undefined") {
      const keys = Object.keys(localStorage).filter((k) =>
        k.startsWith("clauseai_"),
      );
      console.log("[dash INIT] localStorage keys:", keys);
      keys.forEach((k) => console.log(`  ${k} =`, localStorage.getItem(k)));
    }
    console.log("[dash INIT] setting 600ms timer...");
    const t = setTimeout(() => {
      console.log("[dash TIMER FIRED] setting rehydrated+ready to true");
      setRehydrated(true);
      setReady(true);
    }, 600);
    return () => {
      console.log("[dash CLEANUP] clearing timer");
      clearTimeout(t);
    };
  }, [dispatch]);
  ...
  useEffect(() => {
    console.log(
      `[dash agreementId effect] rehydrated=${rehydrated} agreementId=${agreementId}`,
    );
    if (!rehydrated) return;
    if (!agreementId) {
      console.warn("[dash] no agreementId — redirecting to /");
      router.replace("/");
    } else {
      console.log("[dash] agreementId ok:", agreementId);
    }
  }, [rehydrated, agreementId, router]);

  useEffect(() => {
    console.log("[dash] ready=", ready);
  }, [ready]);
  useEffect(() => {
    console.log("[dash] rehydrated=", rehydrated);
  }, [rehydrated]);
  useEffect(() => {
    console.log("[dash] Redux state:", {
      agreementId,
      walletAddress,
      isPartyB,
      hasTerms: !!editedTerms,
    });
  }, [agreementId, walletAddress, isPartyB, editedTerms]);

  const poll = useCallback(() => {
    if (!agreementId) {
      console.log("[dash poll] skipped, no agreementId");
      return;
    }
    console.log("[dash poll] polling:", agreementId);
    dispatch(pollAgreementThunk(agreementId))
      .unwrap()
      .then((r) => {
        console.log("[dash poll] success:", r);
        setLastPolled(new Date().toLocaleTimeString());
      })
      .catch((e: unknown) => {
        console.warn("[dash poll] failed (non-fatal):", e);
  ...
  useEffect(() => {
    console.log(
      `[dash poll effect] rehydrated=${rehydrated} agreementId=${agreementId}`,
    );
    if (!rehydrated || !agreementId) return;
    poll();
    const iv = setInterval(poll, POLL_MS);
    return () => clearInterval(iv);
  }, [rehydrated, agreementId, poll]);
  ...
  if (!ready) {
    console.log("[dash] rendering SPLASH");
    return (
      <div className="detect-wrap">
        ...
        >
          <div style={{ color: "#f5c400", fontWeight: 700, marginBottom: 8 }}>
            🔍 Debug Panel
          </div>
          <div>
            ready:{" "}
            <b style={{ color: ready ? "#22c55e" : "#f87171" }}>
              {String(ready)}
            </b>
          </div>
          <div>
            rehydrated:{" "}
            <b style={{ color: rehydrated ? "#22c55e" : "#f87171" }}>
              {String(rehydrated)}
            </b>
          </div>
          <div>
            agreementId:{" "}
            <b style={{ color: agreementId ? "#22c55e" : "#f87171" }}>
              {agreementId ?? "null"}
            </b>
          </div>
          <div>
            wallet:{" "}
            <b style={{ color: walletAddress ? "#22c55e" : "#f87171" }}>
              {walletAddress ? walletAddress.slice(0, 14) + "…" : "null"}
            </b>
          </div>
          <div>
            isPartyB: <b style={{ color: "#94a3b8" }}>{String(isPartyB)}</b>
          </div>
          <div>
            editedTerms:{" "}
            <b style={{ color: editedTerms ? "#22c55e" : "#f87171" }}>
              {editedTerms ? "✓ present" : "null"}
            </b>
          </div>
          <div style={{ marginTop: 8, fontSize: 11, color: "#334155" }}>
            ↑ If ready stays false after 1s, check console
          </div>
        </div>
  ...
  console.log("[dash] rendering MAIN DASHBOARD");
  return (
        ...
        <div className="modal-overlay" onClick={() => setDisputeModal(false)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="modal-body">
              ...
              . Paused for{" "}
              <strong style={{ color: "#f59e0b" }}>48 hours</strong>.
            </p>
