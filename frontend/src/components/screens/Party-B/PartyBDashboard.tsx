"use client";
// ============================================================
// components/partyB/PartyBDashboard.tsx — with Agreement History
//
// Two views:
//   1. "Current" — the active agreement (from Redux state)
//   2. "History" — all agreements Party B has joined on this device
//      fetched via pB_agreements list in localStorage
// ============================================================
...
function HistoryCard({
  ...
  const payerName = (data.terms?.payer ??
    ...
    data.partyA ??
    "Payer") as string;
  const receiverName = (data.terms?.receiver ??
    data.terms?.partyB ??
    data.partyB ??
    "You") as string;
...
export default function PartyBDashboard() {
  ...
  // All Party B agreement IDs from localStorage
  const [historyIds, setHistoryIds] = useState<string[]>([]);

  useEffect(() => {
    const ids = getPartyBAgreementIds();
    setHistoryIds(ids);
  }, []);
          ...
          <>
            ...
            <div className="fade-up d3 info-strip">
              ...
              >
                ...
                via Socket.io when the payer releases funds.
                {historyIds.length > 1 && (
                  <>
                    {" "}
                    Switch to the{" "}
                    ...
                    tab to see all your agreements.
                  </>
                )}
              </p>
            </div>
