...
// Route: /dashboard
// Unified dashboard for BOTH Payer (Party A) and Receiver (Party B).
// Detects role from Redux state → shows the correct panel.
//
// Navigation into this page:
//   Party A: ScreenLockFunds → dispatch(setScreen("dashboard"))
//             → router.push("/dashboard")
//   Party B: /agreement/[id] DepositState → "Go to Dashboard"
//             → router.push("/dashboard")
// ============================================================
...
const POLL_MS = 12_000; // ~1 Stacks block
...
export default function DashboardPage() {
  ...
  const didRedirectGuard = useRef(false);

  // ── Guard: redirect to home if no agreement loaded ───────────
  useEffect(() => {
    if (didRedirectGuard.current) return;
    if (!agreementId) {
      router.replace("/");
      return;
    }
    didRedirectGuard.current = true;
    // Small delay so role-detection splash shows
  ...
  // ── Role detection splash ─────────────────────────────────────
  if (!ready) {
    return (
      <div className="detect-wrap">
        <div className="detect-orb">🔍</div>
        <p className="detect-label">Detecting your role…</p>
          ...
          {!isPartyB && !isFinished && (
            <div className="actions-section">
              ...
              >
                ...
                receiver confirms conditions are met. Dispute if something goes
                wrong.
              </div>
              <DangerBtn
                disabled={anyBusy}
          ...
          {isPartyB && !isFinished && (
            <div className="actions-section">
              ...
              >
                ...
                met to get paid. Dispute if there is an issue.
              </div>
              <PrimaryBtn
...
const splashCSS = `
  ...
  .detect-orb {
    ...
    background: radial-gradient(circle, #f5c40012 0%, transparent 70%);
  ...
  .dot {
    ...
    background:#f5c400;
    animation: dot-up 0.7s ease-in-out infinite alternate;
  }
  @keyframes orb-beat {
    0%,100% { transform:scale(1);   box-shadow:0 0 0 0 rgba(245,196,0,.15); }
    50%      { transform:scale(1.06); box-shadow:0 0 28px 6px rgba(245,196,0,.08); }
  }
  @keyframes dot-up {
...
const css = `
  * { box-sizing: border-box; }
  ...
  .escrow-badge {
    ...
    color:#64748b; background:#161616;
    border:1px solid #2a2a2a; border-radius:99px; padding:3px 10px;
  }
  .role-chip {
    font-size:11px; font-family:var(--font-mono,monospace); font-weight:700;
    border:1px solid; border-radius:99px; padding:4px 12px;
  }
  .state-chip {
    font-size:11px; font-family:var(--font-mono,monospace);
    border:1px solid; border-radius:99px; padding:4px 12px;
    background:#111; display:flex; align-items:center; gap:5px;
  }
  .pulse-dot {
  ...
  .db-left  { display:flex; flex-direction:column; gap:12px; }
  .db-right { display:flex; flex-direction:column; gap:12px; }
  ...
  .role-card-wallet { font-size:11px; font-family:var(--font-mono,monospace); color:#475569; margin-top:4px; }

  /* ── Timeout banner ── */
  .timeout-banner {
    background:#f59e0b10; border:1px solid #f59e0b; border-radius:10px;
  ...
  .timeout-btn {
    background:#f59e0b; color:#0a0a0a; border:none; border-radius:6px;
    padding:7px 14px; font-size:12px; font-weight:700; cursor:pointer;
    display:flex; align-items:center; gap:5px; flex-shrink:0;
  }
  .timeout-btn:disabled { opacity:.5; cursor:not-allowed; }
  ...
  .actions-title {
    ...
    color:#475569; text-transform:uppercase; letter-spacing:.12em;
  }
  .info-box {
    border:1px solid; border-radius:8px;
    padding:10px 14px; font-size:12px; line-height:1.65;
  }
  .primary-btn {
  ...
  .new-agreement-btn:hover { opacity:.88; }

  /* ── Amount card ── */
  .amount-card {
    background:linear-gradient(135deg,#140f00 0%,#111 100%);
    border:1px solid #f5c40022; border-radius:16px;
  ...
  .amount-label { font-size:10px; font-family:var(--font-mono,monospace); color:#f5c400; text-transform:uppercase; letter-spacing:.18em; margin-bottom:10px; }
  .amount-sbtc  { font-size:38px; font-weight:900; color:#f5c400; letter-spacing:-1.5px; line-height:1; }
  .amount-unit  { font-size:13px; font-family:var(--font-mono,monospace); color:#f5c400; margin-top:3px; }
  .amount-usd   { font-size:15px; color:#fff; margin-top:8px; font-weight:600; }
  .amount-micro { font-size:10px; font-family:var(--font-mono,monospace); color:#334155; margin-top:6px; }
  ...
  .section-label { font-size:10px; font-family:var(--font-mono,monospace); color:#475569; text-transform:uppercase; letter-spacing:.1em; }
  .polled-label  { font-size:9px; font-family:var(--font-mono,monospace); color:#334155; }
  ...
  .tc-value { font-size:13px; font-weight:700; font-family:var(--font-mono,monospace); }
  ...
  .terms-header { padding:10px 16px; border-bottom:1px solid #1a1a1a; font-size:10px; font-family:var(--font-mono,monospace); color:#475569; text-transform:uppercase; letter-spacing:.1em; }
  ...
  .terms-key { font-size:11px; font-family:var(--font-mono,monospace); color:#475569; flex-shrink:0; }
  ...
  .party-role { font-size:9px; font-family:var(--font-mono,monospace); text-transform:uppercase; letter-spacing:.12em; margin-bottom:4px; display:flex; align-items:center; gap:5px; }
  .party-you  { border-radius:4px; padding:1px 5px; font-size:8px; }
  .party-name   { font-size:13px; font-weight:700; }
  .party-status { font-size:10px; font-family:var(--font-mono,monospace); margin-top:4px; }
  .party-wallet { font-size:9px; font-family:var(--font-mono,monospace); color:#334155; margin-top:3px; }
  ...
  .modal-box {
    background:#111; border:1px solid #7f1d1d; border-radius:16px;
  ...
  @media (max-width: 780px) {
    .db-grid { grid-template-columns: 1fr; padding: 16px; }
